...
def get_train_args(args: Optional[Dict[str, Any]] = None) -> _TRAIN_CLS:
    ...
    # Save Hparams
    from llamafactory.webui.utils import save_args
    from dataclasses import asdict
    output_args = {}
    output_args.update(asdict(model_args))
    output_args.update(asdict(data_args))
    output_args.update(training_args.to_dict())
    output_args.update(asdict(finetuning_args))
    output_args.update(generating_args.to_dict())
    if not os.path.exists(training_args.output_dir):
        os.makedirs(training_args.output_dir, exist_ok=True)
    save_args(os.path.join(training_args.output_dir, "hparams.yaml"), output_args)
